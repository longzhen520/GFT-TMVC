function [Z_hat,Fin] =GTNN_MVC_without_GFT(X,opts)
K=length(X);
N=size(X{1},2);
for k=1:K
...
%   Z_hat=prox_nuclear(Dit,beta/rho3);     
    ...
    W_tensor = cat(3, Q2{:,:});
    %% 
    G_tensor=permute(prox_tnn(permute(Z_tensor + 1/rho2*W_tensor,[3,1,2]),gamma/rho2),[2,3,1]);  
        for k=1:K
    G{k} = G_tensor(:,:,k);
        end
...
%     rho3 = min(rho3*miu, max_rho);
%     KK=max(floor(KK*0.95),cls_num);

% Pho1(iter)=rho1;
% Pho2(iter)=rho2;
% Pho3(iter)=rho3;
% for k=1:K
%     C = SpectralClustering(0.5*(abs(Z{k})+abs(Z{k})'),cls_num);
%    ACC_Z1(iter,k) = Accuracy(C,double(gt));
% end
% C = SpectralClustering(0.5*(abs(Z_hat)+abs(Z_hat')),cls_num);
% ACC_Z(iter) = Accuracy(C,double(gt));
% % figure(1),plot(1:iter,ACC_Z),hold on;
% end
% max_ACC=max(ACC_Z(:));
end